...
export default function LoginPage() {
	...
	const [signUpPageActive, setSignupPageActive] = React.useState<boolean>(false);

	return (
		<div className={css["login-root"]}>
			<div className={clsx([css["container"], signUpPageActive && css["right-panel-active"]])} id="container">
				<div className={clsx([css["form-container"], css["sign-up-container"]])}>
					<form action="#">
						<h1 className="text-black text-3xl font-bold">Create Account</h1>
						<div className={clsx([css["social-container"], "flex justify-around w-6/12"])}>
				...
				<div className={css["overlay-container"]}>
					<div className={css["overlay"]}>
						<div className={[css["overlay-panel"], css["overlay-left"]].join(' ')}>
							...
							<Button outlined={true} label="Sign In" onClick={() => setSignupPageActive(false)} />
						</div>
						<div className={[css["overlay-panel"], css["overlay-right"]].join(' ')}>
							<h1 className="text-3xl font-bold">Hello, Friend!</h1>
							<p>Enter your personal details and start journey with us</p>
							<Button outlined={true} label="Sign Up" onClick={() => setSignupPageActive(true)} />
						</div>
					</div>
				</div>
			</div>
